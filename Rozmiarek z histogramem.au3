	...
	For $t = 1 To 15
		...
		Switch $nMsg
			Case $GUI_EVENT_CLOSE
				...
				_GDIPlus_GraphicsDispose($uchwyt_bitmapy)
				Exit
			Case $Button1
				Exit
	...
	If _IsPressed(43) Then
		If _ClipBoard_IsFormatAvailable($CF_BITMAP) == 1 Then
			If _ClipBoard_Open(0) == 1 Then
				...
				$liczba_kolorowych_pixeli = 0
				For $i = 0 To $rozmiarX - 1
					For $j = 0 To $rozmiarY - 1
						;ConsoleWrite(_GDIPlus_BitmapGetPixel($uchwyt_bitmapy,$i,$j) & @CRLF)
						If (_GDIPlus_BitmapGetPixel($uchwyt_bitmapy, $i, $j) <> 4294967295) Then
							$liczba_kolorowych_pixeli = $liczba_kolorowych_pixeli + 1
						EndIf
					Next
				Next
				_GDIPlus_BitmapDispose($uchwyt_bitmapy)
				...
				ToolTip("Powierzchnia pixeli kolorowych = " & Round( $liczba_kolorowych_pixeli * 0.2645833333333 * 0.2645833333333,2) & "mm2" & @CRLF & "Powierzchnia= " & Round($rozmiarX * 0.2645833333333 * $rozmiarY * 0.2645833333333, 2) & "mm2" & @CRLF, -1, -1)

				ConsoleWrite("Powierzchnia pixeli kolorowych = " & $liczba_kolorowych_pixeli * 0.2645833333333 * 0.2645833333333 & "mm2" & @CRLF)
